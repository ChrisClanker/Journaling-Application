...
from .models import JournalEntry, Blurb, Report, Goal, Tag, JournalTemplate, JournalFeedback
import json
import re
...
# Rule-based suggestion mappings for when USE_AI=False
MOOD_SUGGESTION_MAP = {
    # Sad/disappointed/ashamed -> self-care
    'sad': 'Consider reaching out to someone you trust tomorrow. Sometimes sharing how you feel can lighten the load.',
    'disappointed': 'Consider reaching out to someone you trust tomorrow. Sometimes sharing how you feel can lighten the load.',
    'ashamed': 'Consider reaching out to someone you trust tomorrow. Sometimes sharing how you feel can lighten the load.',
    # Happy/excited/grateful/proud -> positive momentum
    'happy': "You are in a great place! Consider what contributed to this positive day and try to recreate those conditions tomorrow.",
    'excited': "You are in a great place! Consider what contributed to this positive day and try to recreate those conditions tomorrow.",
    'grateful': "You are in a great place! Consider what contributed to this positive day and try to recreate those conditions tomorrow.",
    'proud': "You are in a great place! Consider what contributed to this positive day and try to recreate those conditions tomorrow.",
    # Angry/frustrated/envious -> physical activity
    'angry': 'Take some time for physical activity tomorrow - a walk, run, or workout can help process these feelings.',
    'frustrated': 'Take some time for physical activity tomorrow - a walk, run, or workout can help process these feelings.',
    'envious': 'Take some time for physical activity tomorrow - a walk, run, or workout can help process these feelings.',
    # Afraid/worried/anxious -> worry management
    'afraid': "Try writing down your top 3 worries tomorrow morning, then identify one small action for each. Breaking things down helps.",
    'worried': "Try writing down your top 3 worries tomorrow morning, then identify one small action for each. Breaking things down helps.",
    'anxious': "Try writing down your top 3 worries tomorrow morning, then identify one small action for each. Breaking things down helps.",
    # Calm/relieved/hopeful -> planning
    'calm': "This is a good time for planning. Consider setting a small intention for tomorrow that builds on this peaceful state.",
    'relieved': "This is a good time for planning. Consider setting a small intention for tomorrow that builds on this peaceful state.",
    'hopeful': "This is a good time for planning. Consider setting a small intention for tomorrow that builds on this peaceful state.",
}

DEFAULT_SUGGESTION = "Take a moment tomorrow morning to check in with yourself. What's one small thing you'd like to focus on today?"


def generate_rule_based_suggestions(entry):
    """Generate rule-based suggestions from mood keywords and content patterns."""
    moods = []
    if entry.mood:
        try:
    ...
    # Check each mood against the suggestion map
    for mood in moods:
        mood_lower = mood.lower()
        if mood_lower in MOOD_SUGGESTION_MAP:
            suggestion = MOOD_SUGGESTION_MAP[mood_lower]
            # Determine tone based on mood category
            if mood_lower in ('sad', 'disappointed', 'ashamed'):
                tone = 'encouraging'
            elif mood_lower in ('happy', 'excited', 'grateful', 'proud'):
                tone = 'encouraging'
            elif mood_lower in ('angry', 'frustrated', 'envious'):
                tone = 'action-oriented'
            elif mood_lower in ('afraid', 'worried', 'anxious'):
                tone = 'reflective'
            elif mood_lower in ('calm', 'relieved', 'hopeful'):
                tone = 'action-oriented'
            break
...
def generate_ai_suggestions(entry):
    """Generate AI-powered suggestions via Ollama."""
    prompt = (
        "You are a thoughtful, empathetic journaling companion. "
        "The user has just written a journal entry. Based on their content, mood, "
        "reflections, and gratitude, provide 2-3 brief, actionable suggestions for "
        "things they might consider doing or focusing on tomorrow. "
        "Be specific to their situation, not generic. "
        "Keep it under 200 words. Format as markdown with bullet points."
    )

    context_parts = []
    if entry.content:
        context_parts.append(f"Journal Content:\n{entry.content}")
    if entry.mood:
        context_parts.append(f"Mood: {entry.mood}")
    if entry.reflections:
        context_parts.append(f"Reflections:\n{entry.reflections}")
    if entry.gratitude:
        context_parts.append(f"Gratitude:\n{entry.gratitude}")
    ...
    suggestions = response_data.get('response', '').strip()

    # Determine tone from the response
    tone = 'reflective'
    suggestions_lower = suggestions.lower()
    if any(w in suggestions_lower for w in ['great', 'wonderful', 'amazing', 'keep it up', 'proud']):
        tone = 'encouraging'
    elif any(w in suggestions_lower for w in ['walk', 'run', 'exercise', 'activity', 'move']):
        tone = 'action-oriented'

    return suggestions, tone


def get_or_create_feedback(entry):
    """Get existing feedback or generate new one for a journal entry."""
    feedback, created = JournalFeedback.objects.get_or_create(
        entry=entry,
        defaults={
            'suggestions': '',
            'tone': 'reflective',
    ...
    if created:
        if settings.USE_AI:
            try:
                suggestions, tone = generate_ai_suggestions(entry)
            except Exception:
                suggestions, tone = generate_rule_based_suggestions(entry)
        else:
            suggestions, tone = generate_rule_based_suggestions(entry)
...
@login_required
def journal_feedback(request, id):
    """Show feedback/suggestions for a journal entry."""
    entry = get_object_or_404(JournalEntry, id=id, user=request.user)
    feedback = get_or_create_feedback(entry)
    return render(request, 'journal_feedback.html', {
        'entry': entry,
        'feedback': feedback,
    })
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for blurb in Blurb.objects.filter(user=request.user, journalEntry=None):
                ...
                blurb.save()
            # redirect to feedback page for the new entry
            return HttpResponseRedirect(f"/feedback/{new.id}/")
